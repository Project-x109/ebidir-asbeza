...
if (isset($_SESSION['id'])) {
    ...
    // SQL query to fetch loan data for the last two years without the provider condition
    $sql = "SELECT YEAR(createdOn) AS loanYear, MONTH(createdOn) AS loanMonth, COUNT(*) AS loanCount 
            FROM loans 
            WHERE YEAR(createdOn) BETWEEN ? AND ?
    ...
    // Bind parameters
    $stmt->bind_param("ii", $lastYear, $currentYear);
