                            ...
                            <div class="col-md-6 col-lg-12 col-xl-12 order-0 mb-4">
                                <div class="card">
                                    ...
                                    <div class="table-responsive text-nowrap ms-3 me-3">
                                        ...
                                        if (!empty($search)) {
                                            $countQuery = "SELECT COUNT(*) as total FROM loans INNER JOIN users ON users.user_id = loans.user_id WHERE loans.status != 'paid' AND loans.status != 'approved' AND loans.provider = 'SELF' AND (users.name LIKE ? OR loans.user_id LIKE ? OR  loans.price LIKE ? OR loans.credit_score LIKE ? OR loans.createdOn LIKE ?)";
                                            $stmtCount = $conn->prepare($countQuery);
                                            $searchParam = "%$search%";
                                            $stmtCount->bind_param("sssss", $searchParam,$searchParam, $searchParam, $searchParam, $searchParam);
                                        } else {
                                            $countQuery = "SELECT COUNT(*) as total FROM loans INNER JOIN users ON users.user_id = loans.user_id WHERE loans.status != 'paid' AND loans.status != 'approved' AND loans.provider = 'SELF'";
                                            $stmtCount = $conn->prepare($countQuery);
                                        }
                                        ...
                                        if (!empty($search)) {
                                            $sql = "SELECT *, loans.user_id AS userID, loans.status AS status, loans.id AS loan_id FROM loans INNER JOIN users ON users.user_id = loans.user_id WHERE loans.status != 'paid' AND loans.status != 'approved' AND loans.provider = 'SELF' AND (users.name LIKE ? OR loans.user_id LIKE ? OR loans.price LIKE ? OR loans.credit_score LIKE ? OR loans.createdOn LIKE ?) LIMIT ?, ?";
                                            $stmt = $conn->prepare($sql);
                                            $searchParam = "%$search%";
                                            $stmt->bind_param("sssssii", $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $offset, $recordsPerPage);
                                        } else {
                                            $sql = "SELECT *, loans.user_id AS userID, loans.status AS status, loans.id AS loan_id FROM loans INNER JOIN users ON users.user_id = loans.user_id WHERE loans.status != 'paid' AND loans.status != 'approved' AND loans.provider = 'SELF' LIMIT ?, ?";
                                            $stmt = $conn->prepare($sql);
